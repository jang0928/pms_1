 ...
 */
function GCGantt(elementId, tasks, resources, options) {

   var _gcGanttInstance = this;

   if (!(tasks instanceof Array)) {
      console.error("Invalid arguments to task: expected an array of tasks.");
      return;
   }
   ...
   var _options = {
      showStartDate: this.getOption(options, 'showStartDate', true), // Show start date column
      showEndDate: this.getOption(options, 'showEndDate', true), // Show end date column
      showResources: this.getOption(options, 'showResources', true), //Show resources column
      showResourceFilter: true, // Show resource filter
      title: {
   ...
   var _elementGcGantt = document.getElementById(_elementId);
   _elementGcGantt.setAttribute("style", "position: relative !important;");

   // assing resources
   var _resources = new Array();
   if (_options.showResources) {
      if (!(resources instanceof Array)) {
         console.error("Invalid resources: expected an array of resources.");
         return;
      }

      // build an indexed array as resources
      for (var i in resources) {
         _resources[resources[i].id] = resources[i].resource;
      }
   }
   ...
   this.getResourceById = function (id) {
      ...
      if (_resources[id] === undefined) {
         throw "ERROR: The resource id " + id + " is not found.";
      }
   ...
   this.buildHeader = function () {
      ...
      if (_options.showResources) {
         var _div = this.createDiv(_options.title.resources, "gc-task-resources");
         _row.appendChild(_div);
      }
      if (_options.showResourceFilter) {
         // TODO - Build filter 
      }

      for (var i in _arrayDates) {
         _row.appendChild(this.createDiv((_arrayDates[i].getMonth() + 1) + "/" + _arrayDates[i].getDate(), "gc-task-header-day"));
      }
      _elementGcGantt.appendChild(_row);
   };

   this.fixSize = function () {
      var pos = _elementGcGantt.getBoundingClientRect();
      _elementGcGantt.style.width = pos.width + "px";
   };

   this.showHideRows = function (row, hidding) {
      var _parent = row.getAttribute("gc-id").split("-");
      var _elements = getElementsByAttribute("gc-parent", _parent[1], _elementGcGantt);
      for (var i in _elements) {
         hidding = hidding == null ? _elements[i].style.display != 'none' : hidding;
         if (hasClass(_elements[i], "gc-task-group")) {
            // if this line is a group row, collapse it
            this.showHideRows(_elements[i], hidding);
         }
         _elements[i].style.display = !hidding ? '' : 'none';
      }
      var _desc = getElementByAttribute("gc-task-id", _parent[1], row);
      if (hidding) {
         // TODO: change replace "icon"
         _desc.innerHTML = _desc.innerHTML.replace("-", "+");
      } else {
         _desc.innerHTML = _desc.innerHTML.replace("+", "-");
      }
      // TODO: optimize hide/show tasks process
      this.repaint();
   };
   ...
   this.expandAll = function () {
      // get all groups
      var groupRows = _elementGcGantt.getElementsByClassName("gc-task-group");
      for (var i = 0; i < groupRows.length; i++) {
         this.showHideRows(groupRows[i], false);
      }
   };
   ...
   this.collapseAll = function () {
      ...
      var groupRows = _elementGcGantt.getElementsByClassName("gc-task-group");
      for (var i = 0; i < groupRows.length; i++) {
         this.showHideRows(groupRows[i], true);
      }
   };
   ...
   this.buildBody = function (tasks, level, parentTask) {
      ...
      for (var _i = 0; _i < level; _i++) {
         // add some spaces according to the level of nested row
         _prefix = "\u00A0\u00A0\u00A0\u00A0";
      }
      // create all tasks on chart
      for (var i in tasks) {
         _plusGroup = '';
         if (tasks[i] instanceof GCTaskGroup) {
            _plusGroup += "+ ";
         ...
         _div.setAttribute("description", tasks[i].getDescription());
         _div.setAttribute("gc-task-id", tasks[i].getId());
         if (tasks[i] instanceof GCTaskGroup) {
            _div.onclick = function () {
               _gcGanttInstance.showHideRows(this.parentNode);
               _gcGanttInstance.onTaskNameClick(this.getAttribute("gc-task-id"), true);
            };
         } else {
            _div.onclick = function () {
               _gcGanttInstance.onTaskNameClick(this.getAttribute("gc-task-id"), false);
            };
         }
         _row.appendChild(_div);
         ...
         // check if column start date is enable
         if (_options.showStartDate) {
            var _div = this.createDiv(tasks[i].getStartDate().toLocaleString().slice(0, 11), "gc-task-start-date");
            _row.appendChild(_div);
         }
         // check if column end date is enable
         if (_options.showEndDate) {
            var _div = this.createDiv(tasks[i].getEndDate().toLocaleString().slice(0, 11), "gc-task-end-date");
            _row.appendChild(_div);
         }
         // check if column resources is enable
         if (_options.showResources) {
            if (tasks[i] instanceof GCTask && tasks[i].getResources().length > 0) {
               // get all resource names
               var resouceArray = new Set(tasks[i].getResources());
               var resouceNames = '';
               console.log(resouceArray);
               resouceArray.forEach(function(resourceId){
                  resouceNames += this.getResourceById(resourceId) + "\n";
               }, this);
               // add resource names on gantt
               var _div = this.createDiv(resouceNames, "gc-task-resources", false);
               _row.appendChild(_div);
            } else {
   ...
   this.onTaskClick = function (taskId, isGroup) {
      console.log("onTaskClick", taskId, isGroup);
   };

   this.onTaskNameClick = function (taskId, isGroup) {
      console.log("onTaskNameClick", taskId, isGroup);
   };

   /**
    * Draw all tasks on gantt chart
    * 
    * @param {type} tasks
    */
   this.drawTasks = function (tasks) {
      for (var i in tasks) {
         ...
         if (tasks[i] instanceof GCTaskGroup) {
            //build task group
            _rowTask = getElementByAttribute("gc-id", "group-" + tasks[i].getId(), _elementGcGantt);
            ...
            //build task
            _rowTask = getElementByAttribute("gc-id", "task-" + tasks[i].getId(), _elementGcGantt);
         ...
         node.onclick = function () {
            _gcGanttInstance.onTaskClick(this.getAttribute("gc-task-id"), (tasks[i] instanceof GCTaskGroup));
         };
         ...
         var _ganttPositions = _elementGcGantt.getBoundingClientRect();
         _styleStr += "height:" + (pos.height - 4) + "px;";
         ...
         if (tasks[i] instanceof GCTaskGroup) {
            ...
            this.drawTasks(tasks[i].getTasks());
         } else {
            //create a task
            _rowTask.appendChild(node);
   ...
   /**
    * Build gantt with all parameters
    * @returns {undefined}
    */
   this.buildGantt = function () {
      _elementGcGantt.classList.add("gc-gantt");
      this.buildHeader();
      this.buildBody(_tasks, 0, null);
      this.drawTasks(_tasks);
      this.fixSize();
   };

   /**
    * Redraw all gantt chart
    * 
    */
   this.repaint = function () {
      var _bars = _elementGcGantt.getElementsByClassName("gc-task-bar");
      while (_bars.length > 0) {
         _bars[0].parentNode.removeChild(_bars[0]);
      }

      _bars = _elementGcGantt.getElementsByClassName("gc-group-bar");
      while (_bars.length > 0) {
         _bars[0].parentNode.removeChild(_bars[0]);
      }
      this.drawTasks(_tasks);
   ...
   /**
    * Parse all tasks from JSON to GCTaskGroup/GCTask objects
    * @param {type} _jsonTasks
    * @returns {Array of GCTaskGroup|GCTask}
    */
